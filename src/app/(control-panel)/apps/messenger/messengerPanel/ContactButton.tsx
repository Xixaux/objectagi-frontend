...
function ContactButton(props: ContactButtonProps) {
  ...
  if (!contact || !contact.id) {
    console.error('[ContactButton] Invalid contact data', { contact });
    return null;
  }

  const handleClick = () => {
    console.log('[ContactButton] Clicked contact:', { id: contact.id, name: contact.name, selectedChatId });
    onClick(contact.id);
  };

  return (
    <Root title={contact.name || 'Unknown'} placement="left" active={selectedChatId === contact.id ? 1 : 0}>
      <Button
        onClick={handleClick}
        className={clsx(
