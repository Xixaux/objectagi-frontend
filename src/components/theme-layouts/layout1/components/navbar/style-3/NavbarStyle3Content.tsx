...
const StyledPanel = styled(ObjectAGIScrollbars)<StyledPanelProps>(({ theme }) => ({
	backgroundColor: theme.palette.background.default,
	color: theme.palette.text.primary,
...
function NavbarStyle3Content(props: NavbarStyle3ContentProps) {
	...
	const [selectedNavigation, setSelectedNavigation] = useState<ObjectAGINavItemType[]>([]);
	...
	function handleParentItemClick(selected: ObjectAGINavItemType) {
	...
	return (
		<ClickAwayListener onClickAway={() => setPanelOpen(false)}>
			<Root className={clsx('flex h-full flex-auto', className)}>
				<div
					id="objectagi-navbar-side-panel"
