...
const Root = styled(Box)(({ theme }) => ({
    '& > .objectagi-list-item': {
        ...
        // THIS WAS THE KILLER — NOW IT'S DEAD
        '&.type-divider': {
...
export type ObjectAGINavVerticalTabProps = Omit<ObjectAGINavigationProps, 'navigation'> & ObjectAGINavItemComponentProps;

/**
 *  The `ObjectAGINavVerticalTab` component renders vertical navigation item with an adaptable
 *  layout to be used within the `ObjectAGINavigation`. It only supports the `type`s of 'item',
 *  'selection' and 'divider'
 * */
function ObjectAGINavVerticalTab(props: ObjectAGINavVerticalTabProps) {
            ...
            <Root sx={item.sx}>
                <ListItemButton
                    ...
                    className={clsx(
                        ...
                        'objectagi-list-item flex flex-col items-center justify-center p-3'
                    )}
                    ...
                    {dense ? (
                        ...
                        >
                            <div className="relative flex h-8 min-h-8 w-8 items-center justify-center">
                                {item.icon ? (
                                    <ObjectAGISvgIcon
                                        className={clsx('objectagi-list-item-icon', item.iconClass)}
                                        color="action"
                                    >
                                        {item.icon}
                                    </ObjectAGISvgIcon>
                        ...
                        <>
                            ...
                            <ListItemText
                                className="objectagi-list-item-text w-full grow-0 px-2"
                                primary={item.title}
                                classes={{
                                    primary:
                                        'text-md font-medium objectagi-list-item-text-primary truncate text-center truncate'
                                }}
                            />
                        </>
                    )}
...
const NavVerticalTab = ObjectAGINavVerticalTab;
