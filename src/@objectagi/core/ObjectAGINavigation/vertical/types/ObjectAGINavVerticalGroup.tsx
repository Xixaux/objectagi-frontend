...
/**
 * ObjectAGINavVerticalGroup is a component used to render a group of navigation items in a vertical layout.
 */
function ObjectAGINavVerticalGroup(props: ObjectAGINavItemComponentProps) {
		...
		() => (
			<>
				<Root
					...
					className={clsx('objectagi-list-subheader flex items-center py-2.5', !item.url ? 'cursor-default' : '')}
					onClick={() => onItemClick && onItemClick(item)}
					sx={item.sx}
					{...itemProps}
				>
					<ListItemText
						className="objectagi-list-subheader-text"
						sx={(theme) => ({
							margin: 0,
...
const NavVerticalGroup = ObjectAGINavVerticalGroup;
